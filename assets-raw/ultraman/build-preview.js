...
async function buildSinglePreview(entry) {
  const sourcePath = path.join(sourceRoot, `${entry.slug}${entry.sourceExt || ".jpg"}`);
  const outputDir = path.join(previewRoot, entry.era);
...
async function buildBoard(entries) {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    composites.push({ input: fs.readFileSync(entry.outputPath), left, top });
  }
...
async function main() {
  ...
  for (const entry of manifest) {
    builtEntries.push(await buildSinglePreview(entry));
  }
