...
let CANVAS_SIZE = 64;
let PREVIEW_DIAMETER = 52;
let PREVIEW_CENTER = CANVAS_SIZE * 0.5;
...
function getBackgroundStarExclusionRadius(frame) {
  ...
  let radius = PREVIEW_DIAMETER * frame.sizeScale * 0.5 + 5 * scale;
  if (frame.preset.id === "gas_giant_2") {
    radius += 4 * scale;
  }
  return clamp(radius, 22 * scale, 30 * scale);
}
...
function renderBackgroundStars(buffer, frame) {
  ...
  const cacheKey = `${starSeed}:${frame.preset.id}:${frame.config.size}:${frame.config.planetX}:${frame.config.planetY}:${CANVAS_SIZE}`;
  ...
  if (!background) {
    ...
    const starCount = Math.round(10 * (CANVAS_SIZE / 64) * (CANVAS_SIZE / 64));
    let attempts = 0;
    while (background.length < starCount && attempts < 400 * (CANVAS_SIZE / 64)) {
      attempts += 1;
...
function buildFrameState(config, preset, progressValue) {
  ...
  return {
    config,
    preset,
    centerX: config.planetX * (CANVAS_SIZE / 64),
    centerY: config.planetY * (CANVAS_SIZE / 64),
    progress,
    directionFactor,
    motionFactor,
    spinFactor,
...
function setCanvasSize(size) {
  CANVAS_SIZE = size;
  PREVIEW_DIAMETER = Math.round(size * 52 / 64);
  PREVIEW_CENTER = size * 0.5;
  ACTIVE_PREVIEW_CENTER_X = PREVIEW_CENTER;
  ACTIVE_PREVIEW_CENTER_Y = PREVIEW_CENTER;
  // Clear caches since they depend on canvas size
  PREVIEW_CACHE.clear();
  STARFIELD_CACHE.clear();
}
